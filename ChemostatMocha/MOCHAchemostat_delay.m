function [Lset,alphaset,biol] = MOCHAchemostat_delay(tset,ic,D,L0,I0,B0,r,yGM,uCL,uNI,uCB,uNB,qC,qN,qG,attack,kappa0,kappaI,kappaB,kappaP,kappaV,kappaM,c)
...
for i = 1:length(tset)
   alphaset(i,:) = mixo_opt(Lset(i),biol(i,5),biol(i,4)); 
    ...
    function system = ecosysmodel_T(~,x)
        ...
        aV = x(6);  % investment in vacuoles
        aP = x(7);  % investment in plastids
        aM = x(8);  % investment in machinery
        ...
        alpha_opts = mixo_opt(L,I,B);
        
        daVdt = c * (alpha_opts(1) - aV);
        daPdt = c * (alpha_opts(2) - aP);
        daMdt = c * (alpha_opts(3) - aM);
        ...
        system = [dVdt; dPdt; dMdt; dBdt; dIdt; daVdt; daPdt; daMdt];
    end
